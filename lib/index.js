...
export default function GhostContentAPI({url, host, ghostPath = 'ghost', version, key}) {
    ...
    const api = ['posts', 'authors', 'tags', 'pages', 'settings'].reduce((apiObject, resourceType) => {
        ...
        function search(data, options = {}, memberToken) {
            if (!data || !data.search) {
                return Promise.reject(new Error(`${name} search requires a search.`));
            }

            const params = Object.assign({}, data, options);

            return makeRequest(resourceType, params, data.id || `search/${data.search}`, memberToken);
        }
